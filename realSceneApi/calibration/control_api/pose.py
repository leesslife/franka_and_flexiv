...
from control_api.rfvector import compute_norm, compute_quat_u2v, normalize
...
# 欧拉角转四元数 
def RPY2Quar(euler):   #euler[0] yaw, euler[1] pitch,euler[2] roll
    cy=math.cos(euler[0]*0.5)
    sy=math.sin(euler[0]*0.5)
    cp=math.cos(euler[1]*0.5)
    sp=math.sin(euler[1]*0.5)
    cr =math.cos(euler[2] * 0.5)
    sr =math.sin(euler[2] * 0.5)
